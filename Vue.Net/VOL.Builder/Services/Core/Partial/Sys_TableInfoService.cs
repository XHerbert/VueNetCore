...
namespace VOL.Builder.Services
{
    public partial class Sys_TableInfoService
    {
        ...
        {
            //取enable=1的项
            var treeData = await repository.FindAsIQueryable(x => x.Enable == 1)
                .Select(c => new
                {
                    id = c.Table_Id,
                    pId = c.ParentId,
...
DISTINCT
	...
	SELECT CASE WHEN t.ColumnType IN ('DECIMAL','smallmoney','money') THEN 
        ...
        private int InitTable(int parentId, string tableName, string columnCNName, string nameSpace, string foldername, int tableId, bool isTreeLoad)
        {
            ...
            tableId = repository.Find(x => x.TableTrueName == tableName, s => s.Table_Id).FirstOrDefault();
            if (tableId > 0)
                return tableId;
        ...
        public object LoadTable(int parentId, string tableName, string columnCNName, string nameSpace, string foldername, int tableId, bool isTreeLoad)
        {
            ...
            if (!isTreeLoad)
            {
                //重新load之前已经存在的数据表后，应该将Enable改为1
                tableInfo.Enable = 1;
                if(tableInfo.TableColumns != null)
                {
                    tableInfo.TableColumns.ForEach(c => c.Enable = 1);
                }
                repository.UpdateRange<Sys_TableColumn>(tableInfo, true, false, x => new { x.Enable }, d => new { d.Enable }, true);
            }                                   
            if (tableInfo.TableColumns != null)
            {
                //删除表字段时进行软删除，这样可以保留原有已配置好的数据，再次生成后可不必重新配置 先按序号排序 再按是否数据列排序 by xuhbd 2021.1.4
                tableInfo.TableColumns = tableInfo.TableColumns.Where(x => x.Enable == 1).OrderByDescending(x => x.OrderNo).OrderByDescending(x => x.IsColumnData).ToList();
            }
        ...
        public async Task<WebResponseContent> DelTree(int table_Id)
        {
            ...
            //增加Enable过滤，如果存在有效记录则不允许删除 by xuhbd 2021.1.4
            if (tableInfo.TableColumns != null && tableInfo.TableColumns.Where(x=>x.Enable ==1).ToList().Count > 0)
            {
            ...
            //调整为软删除 by xuhbd 2021.1.4
            //repository.Delete(tableInfo, true);
            tableInfo.Enable = 0;
            repository.Update(tableInfo, x => new { x.Enable }, true);
